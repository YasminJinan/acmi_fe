@php
    ...
    $navLinks = [
        ['id' => 'id.dewan',   'en' => 'en.board',    'label' => __('messages.nav_board')],
        ['id' => 'id.produk',  'en' => 'en.products', 'label' => __('messages.nav_products')],
        ['id' => 'id.events',  'en' => 'en.events',   'label' => __('messages.nav_event')],
        ['id' => 'id.galeri',  'en' => 'en.gallery',  'label' => __('messages.nav_gallery')],
        ['id' => 'id.manajer', 'en' => 'en.manager',  'label' => __('messages.nav_manager')],
        ['id' => 'id.artikel', 'en' => 'en.ontopic',  'label' => __('messages.nav_ontopic')],
    ];
@endphp
...
<div x-data="{
    ...
    <nav aria-label="Navigasi utama"
        ...
        <div class="hidden lg:flex items-center gap-8 text-sm font-poppins text-gray-700 dark:text-gray-200">
            @foreach ($navLinks as $link)
                <a href="{{ route($locale === 'id' ? $link['id'] : $link['en']) }}"
                    class="hover:text-orange-500 dark:hover:text-orange-400 transition">{{ $link['label'] }}</a>
            @endforeach
        </div>
        ...
        <div class="flex items-center gap-2 md:gap-3">
            ...
            <a href="{{ $locale === 'id' ? route('id.gabung') : route('en.join') }}"
    ...
    <div id="mobile-menu" x-show="mobileMenuOpen" x-cloak @click.outside="mobileMenuOpen = false"
        ...
        <div class="flex flex-col gap-4 text-center font-poppins text-gray-700 dark:text-gray-200">
            @foreach ($navLinks as $link)
                <a href="{{ route($locale === 'id' ? $link['id'] : $link['en']) }}"
                    class="py-2 hover:text-orange-500 transition @unless($loop->last) border-b border-gray-100 dark:border-white/5 @endunless">{{ $link['label'] }}</a>
            @endforeach

            <div class="flex flex-col gap-2 mt-2 pt-4 border-t border-gray-100 dark:border-white/10">
                <a href="{{ $locale === 'id' ? route('id.gabung') : route('en.join') }}"
                    class="w-full block bg-orange-500 text-white py-3 rounded-2xl font-bold">
                    {{ __('messages.btn_join') }}
                </a>
                <a href="{{ $targetSwitchUrl ?? url('/' . $switchTo) }}"
